...
def run(args):
    ...
    resume_epoch=0
    if args.resume and os.path.exists(args.save_path + 'train.ckpt'):
        checkpoint = torch.load(args.save_path + 'train.ckpt')
        model.module.load_state_dict(checkpoint['state_dict'])
        resume_epoch = checkpoint['epoch'] + 1
        summary_train['epoch'] = resume_epoch
        step = checkpoint['epoch'] + 1
        summary_train['step'] = step
        print( 'Loading model {} from epoch {}' ,format((args.save_path + 'train.ckpt'),str(resume_epoch)))
...
#            step = checkpoint['epoch'] + 1
#            summary_train['step'] = step
#            print( 'Loading model {} from epoch {}' ,format((args.save_path + 'train.ckpt'),str(epoch)))
